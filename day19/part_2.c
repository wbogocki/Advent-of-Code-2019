...
int CheckTractorBeamTest(icv Program[MEMORY_SIZE], int X, int Y)
{
    return CheckTractorBeam(Program, X, Y);
...
int main(void)
{
    ...
    int S = 100; // Santa's ship side size
    ...
    // Find the beam
    X = 5;
    while (!CheckTractorBeamTest(Program, X, Y))
    {
        ++Y;
    }

    // While the bottom left corner of the ship's rectangle is not covered by the beam
    while (!CheckTractorBeam(Program, X - S + 1, Y + S - 1))
    {
        // Track the top edge of the beam
        ++Y;
        while (CheckTractorBeam(Program, X + 1, Y))
        {
            ++X;
        }
        // printf("%dx %dy\t%dx %dy\t%c %c %c\n",
        //        X, Y,
        //        X - S + 1, Y + S - 1,
        //        CheckTractorBeamTest(Program, X, Y) ? '#' : '.',
        //        CheckTractorBeamTest(Program, X + 1, Y) ? '#' : '.',
        //        CheckTractorBeamTest(Program, X - S + 1, Y + S - 1) ? '#' : '.');
    }

    // Move to the top left corner of the ship
    X -= S - 1;

    // It's wrong because we're assuming it's going to touch the top right edge, it doesn't have to

    // Draw the found range
    for (int y = Y; y < Y + S; ++y)
    {
        for (int x = X; x < X + S; ++x)
        {
